#!/usr/bin/env bash
# Build multi-architecture images using cross-compilation
# Faster than QEMU: compilation runs natively, only runtime stage uses emulation
#
# Environment variables:
#   PLATFORMS    - Target platforms (default: linux/amd64,linux/arm64)
#   IMAGE_NAME   - Image name (default: s5)
...
IMAGE_NAME="${IMAGE_NAME:-s5}"
IMAGE_TAG="${IMAGE_TAG:-latest}"
PUSH="${PUSH:-false}"
BUILDER_NAME="s5-multiarch"
...
echo "Containerfile: Containerfile.cross"
...
# --- Podman path ---
build_with_podman() {
    # NOTE: Podman still needs QEMU for the runtime stage (apt-get in debian:bookworm-slim arm64)
    # but this is fast compared to compiling Rust under emulation
    if [ ! -f /proc/sys/fs/binfmt_misc/qemu-aarch64 ]; then
        echo "WARNING: QEMU binfmt not detected for aarch64."
        echo "Cross-compilation handles the build stage natively,"
        echo "but the runtime stage still needs QEMU for arm64 package installation."
        echo "Install with:"
    ...
    for platform in "${ARCH_LIST[@]}"; do
        echo ""
        echo "=== Building for $platform (cross-compilation) ==="
        podman build \
            --platform "$platform" \
            -f Containerfile.cross \
            --manifest "$manifest" \
...
echo ""
echo "Multi-arch build (cross-compilation) complete."
