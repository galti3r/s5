# s5 CI/CD Architecture

## Overview

s5 provides CI/CD pipelines for three platforms, all with the same core stages:
...
| Target | Archive |
|--------|---------|
| `x86_64-unknown-linux-gnu` | `s5-x86_64-linux-gnu.tar.gz` |
| `x86_64-unknown-linux-musl` | `s5-x86_64-linux-musl.tar.gz` |
| `aarch64-unknown-linux-gnu` | `s5-aarch64-linux-gnu.tar.gz` |
| `aarch64-unknown-linux-musl` | `s5-aarch64-linux-musl.tar.gz` |
...
|----------|-------------|
| Shell completions | Bash, Zsh, Fish (`s5 completions <shell>`) |
| Man page | `s5 manpage` generates `s5.1` |
...
The SBOM (`s5.cdx.json`) is included in the GitHub Release for supply chain transparency.
