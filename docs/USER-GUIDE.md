...
```bash
git clone https://github.com/galti3r/s5.git
cd s5
cargo build --release
# Binary is at target/release/s5
```
...
```bash
cargo install --path .
# Binary installed to ~/.cargo/bin/s5
```
...
```bash
make build-static
# Binary is at target/<arch>-unknown-linux-musl/release/s5
```
...
The fastest way to get s5 running is the `quick-start` command. It generates an in-memory configuration and starts the server immediately with zero setup:

```bash
# Start with a specific password
s5 quick-start --password demo

# Start with a custom username
s5 quick-start --username alice --password mysecret

# Start with an auto-generated password (printed to stdout)
s5 quick-start

# Enable standalone SOCKS5 listener
s5 quick-start --password demo --socks5-listen 0.0.0.0:1080
...
Once s5 is running (default port 2222), connect with an SSH client:
...
```bash
s5 ssh-config --user alice --host myserver.example.com --port 2222 --name s5-proxy
```
...
s5 uses TOML for configuration. The default config file path is `config.toml`, overridable via `--config` flag or `S5_CONFIG` environment variable.

```bash
# Use a specific config file
s5 --config /etc/s5/config.toml

# Use S5_CONFIG env var
S5_CONFIG=/etc/s5/config.toml s5
```
...
s5 supports three configuration modes:
...
Requires at minimum `S5_SSH_LISTEN` and authentication credentials. Single-user mode:

```bash
S5_SSH_LISTEN=0.0.0.0:2222 \
S5_PASSWORD_HASH='$argon2id$...' \
S5_USERNAME=alice \
S5_SOCKS5_LISTEN=0.0.0.0:1080 \
s5
```

Multi-user indexed mode (use `S5_USER_<N>_*` pattern, N=0,1,2...):

```bash
S5_SSH_LISTEN=0.0.0.0:2222 \
S5_USER_0_USERNAME=alice \
S5_USER_0_PASSWORD_HASH='$argon2id$...' \
S5_USER_0_ALLOW_SHELL=true \
S5_USER_1_USERNAME=bob \
S5_USER_1_PASSWORD_HASH='$argon2id$...' \
S5_USER_1_ALLOW_SHELL=false \
s5
```

The indexed sequence stops at the first missing index. If `S5_USER_0_USERNAME` is set, flat vars like `S5_USERNAME` are ignored.
...
|----------|-------------|
| `S5_CONFIG` | Config file path |
| `S5_SSH_LISTEN` | SSH listen address |
| `S5_SOCKS5_LISTEN` | Standalone SOCKS5 listen address |
| `S5_HOST_KEY_PATH` | SSH host key file path |
| `S5_LOG_LEVEL` | Log level (trace/debug/info/warn/error) |
| `S5_LOG_FORMAT` | Log format (pretty/json) |
| `S5_MAX_CONNECTIONS` | Max total connections |
| `S5_CONNECTION_TIMEOUT` | Connection timeout (seconds) |
| `S5_IDLE_TIMEOUT` | Idle timeout (seconds) |
| `S5_BAN_ENABLED` | Enable auto-banning (true/false) |
| `S5_BAN_THRESHOLD` | Failed auth attempts before ban |
| `S5_IP_GUARD_ENABLED` | Block private IP connections (true/false) |
| `S5_METRICS_ENABLED` | Enable Prometheus metrics (true/false) |
| `S5_METRICS_LISTEN` | Metrics listen address |
| `S5_API_ENABLED` | Enable management API (true/false) |
| `S5_API_LISTEN` | API listen address |
| `S5_API_TOKEN` | API bearer token |
| `S5_GLOBAL_ACL_DEFAULT_POLICY` | Global ACL policy (allow/deny) |
| `S5_GLOBAL_ACL_DENY` | Comma-separated global deny rules |
| `S5_GLOBAL_ACL_ALLOW` | Comma-separated global allow rules |
| `S5_USERNAME` | Single-user username (default: "user") |
| `S5_PASSWORD_HASH` | Single-user password hash |
| `S5_AUTHORIZED_KEYS` | Comma-separated SSH public keys |

Per-user indexed variables follow the pattern `S5_USER_<N>_<FIELD>`:

| Variable Pattern | Description |
|-----------------|-------------|
| `S5_USER_<N>_USERNAME` | Username |
| `S5_USER_<N>_PASSWORD_HASH` | Password hash |
| `S5_USER_<N>_AUTHORIZED_KEYS` | Comma-separated SSH public keys |
| `S5_USER_<N>_ALLOW_FORWARDING` | Allow SOCKS5/port forwarding |
| `S5_USER_<N>_ALLOW_SHELL` | Allow interactive shell |
| `S5_USER_<N>_MAX_BANDWIDTH_KBPS` | Per-connection bandwidth limit |
| `S5_USER_<N>_SOURCE_IPS` | Comma-separated allowed source CIDRs |
| `S5_USER_<N>_ACL_DEFAULT_POLICY` | Per-user ACL policy |
| `S5_USER_<N>_ACL_ALLOW` | Comma-separated user allow rules |
| `S5_USER_<N>_ACL_DENY` | Comma-separated user deny rules |
| `S5_USER_<N>_TOTP_ENABLED` | Enable TOTP 2FA |
| `S5_USER_<N>_TOTP_SECRET` | TOTP secret (base32) |

### Docker Secrets via _FILE Convention

For sensitive values, s5 supports the `_FILE` suffix convention used by Docker and Kubernetes secrets. Instead of setting the value directly, point to a file containing the secret:

```bash
# Instead of:
S5_API_TOKEN=my-secret-token
...
- `S5_API_TOKEN_FILE`
- `S5_TOTP_SECRET_FILE`
- `S5_USER_<N>_PASSWORD_HASH_FILE`
- `S5_USER_<N>_TOTP_SECRET_FILE`
...
s5 init --preset bastion --username admin --password mysecret --output config.toml

# SOCKS5 proxy preset (forwarding-focused, relaxed shell)
s5 init --preset proxy --username proxyuser --password secret --output config.toml

# Development preset (permissive, debug logging)
s5 init --preset dev --username dev --password dev --output config.toml
...
# Show as JSON
s5 show-config --format json

# From a specific config file
s5 --config /etc/s5/config.toml show-config
```
...
```bash
s5 check-config
s5 --config /etc/s5/config.toml check-config
```
...
s5 uses Argon2id for password hashing, the current recommended algorithm for password storage. Passwords are never stored in plaintext.
...
# Non-interactive
s5 hash-password --password "my-strong-password"
...
s5 supports Time-based One-Time Passwords (TOTP) as a second factor. When enabled, users must append a 6-digit TOTP code to their password.

**Step 1: Generate a TOTP secret**

```bash
s5 generate-totp --username alice
```
...
s5 includes a built-in shell emulator that provides a familiar interactive experience without exposing any real system files or processes. The shell uses a virtual filesystem.
...
```toml
[server]
bookmarks_path = "/var/lib/s5/bookmarks.json"
```
...
```toml
[[webhooks]]
url = "https://hooks.example.com/s5"
...
- Verify s5 is running: `s5 health-check --addr 127.0.0.1:2222`
- Check if the port is in use: `ss -tlnp | grep 2222`
- Verify the `ssh_listen` address in config

**"Permission denied" during SSH login**
- Verify the password hash: `s5 hash-password --password yourpass` and compare
...
- Run `s5 check-config` for detailed error messages
...
```bash
# Via CLI flag
s5 --log-level debug --config config.toml

# Via environment variable
S5_LOG_LEVEL=debug s5
...
s5 health-check --addr 10.0.0.1:2222 --timeout 3
```
