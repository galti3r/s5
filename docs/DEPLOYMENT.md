...
Before deploying s5 to production, review the following checklist:

- [ ] **Strong password hashes**: Generate all password hashes with `s5 hash-password`. Never use weak or example hashes.
...
- [ ] **TLS for SOCKS5**: If the standalone SOCKS5 listener is exposed to the internet, enable TLS:
  ```toml
  [server]
  socks5_tls_cert = "/etc/s5/tls/socks5.crt"
  socks5_tls_key = "/etc/s5/tls/socks5.key"
  ```
...
- [ ] **Audit logging**: Enable and configure the audit log for security event tracking:
  ```toml
  [logging]
  audit_log_path = "/var/log/s5/audit.json"
...
- [ ] **Config validation**: Run `s5 check-config` before deploying any config change.
...
- Depends entirely on use case. s5 adds negligible overhead to the forwarded data stream.
...
s5 ships with a `Containerfile` (aliased as `Dockerfile`) compatible with both Podman and Docker. It uses a multi-stage build with a minimal Debian bookworm-slim runtime image.
...
The image:
- Runs as a non-root `s5` user
- Exposes ports 2222 (SSH), 1080 (SOCKS5), 9090 (metrics), 9091 (API)
- Uses `/etc/s5` as the working directory and volume mount point
- Includes a built-in health check via `s5 health-check`
...
```yaml
volumes:
  - ./config.toml:/etc/s5/config.toml:ro
...
Uses indexed `S5_USER_<N>_*` environment variables with secrets for sensitive values.
...
podman run -d \
  ...
  -p 2222:2222 \
  -p 1080:1080 \
  -e S5_SSH_LISTEN=0.0.0.0:2222 \
  -e S5_SOCKS5_LISTEN=0.0.0.0:1080 \
  -e S5_USER_0_USERNAME=alice \
  -e S5_USER_0_PASSWORD_HASH='$argon2id$v=19$m=19456,t=2,p=1$salt$hash' \
  -e S5_LOG_FORMAT=json \
  -e S5_BAN_ENABLED=true \
  -e S5_IP_GUARD_ENABLED=true \
  -e S5_GLOBAL_ACL_DENY="169.254.169.254:*" \
  -v s5-data:/etc/s5 \
  s5:latest
```
...
```bash
mkdir -p secrets
s5 hash-password --password "alice-secret" > secrets/alice_hash.txt
...
Supported `_FILE` variables: `S5_PASSWORD_HASH_FILE`, `S5_API_TOKEN_FILE`, `S5_TOTP_SECRET_FILE`, and their per-user indexed variants (`S5_USER_<N>_PASSWORD_HASH_FILE`, `S5_USER_<N>_TOTP_SECRET_FILE`).
...
| Path | Purpose | Mode |
|------|---------|------|
| `/etc/s5/config.toml` | Configuration file | `ro` (read-only) |
| `/etc/s5/keys/` or `/etc/s5/host_key` | SSH host key (persisted across restarts) | `rw` |
| `/var/log/s5/` | Audit logs | `rw` |
| `/etc/s5/tls/` | TLS certificates for SOCKS5 | `ro` |
| `/var/lib/s5/bookmarks.json` | Persistent bookmarks | `rw` |
...
# Install
sudo install -m 755 target/release/s5 /usr/local/bin/s5
```
...
```bash
sudo groupadd -r s5
sudo useradd -r -g s5 -d /etc/s5 -s /usr/sbin/nologin s5

sudo mkdir -p /etc/s5 /var/lib/s5 /var/log/s5
sudo chown -R s5:s5 /etc/s5 /var/lib/s5 /var/log/s5
sudo chmod 750 /etc/s5 /var/lib/s5 /var/log/s5
```
...
```bash
# Generate config
sudo -u s5 s5 init --username admin --output /etc/s5/config.toml

# Or copy and edit the example config
sudo cp config.example.toml /etc/s5/config.toml
sudo chown s5:s5 /etc/s5/config.toml
sudo chmod 640 /etc/s5/config.toml
```

### Systemd Unit File

The project includes a hardened systemd unit file at `contrib/s5.service`:

```ini
[Unit]
Description=s5 SSH+SOCKS5 proxy server
Documentation=https://github.com/galti3r/s5
...
User=s5
Group=s5
ExecStart=/usr/local/bin/s5 --config /etc/s5/config.toml
...
# File access
ReadWritePaths=/var/lib/s5 /var/log/s5
ReadOnlyPaths=/etc/s5
...
- `ReadWritePaths`: limited to `/var/lib/s5` (data) and `/var/log/s5` (logs)
- `ReadOnlyPaths=/etc/s5`: config is read-only at runtime
...
s5 logs to stdout/stderr, which systemd captures in the journal:
...
# View recent logs
sudo journalctl -u s5 --since "1 hour ago"

# View logs in JSON format (if configured)
sudo journalctl -u s5 -o cat

# View only error-level entries
sudo journalctl -u s5 -p err
```
...
s5 rotates its audit log file automatically based on configuration:
...
/var/log/s5/*.log {
    ...
    create 0640 s5 s5
    postrotate
        systemctl reload s5 2>/dev/null || true
    endscript
}
```
...
```bash
sudo cp contrib/s5.logrotate /etc/logrotate.d/s5
```
...
Add s5 to your Prometheus configuration:
...
|--------|------|-------------|
| `s5_connections_active` | Gauge | Current number of active connections |
| `s5_connections_total` | Counter | Total connections since start |
| `s5_bytes_sent_total` | Counter | Total bytes sent (per user label) |
| `s5_bytes_received_total` | Counter | Total bytes received (per user label) |
| `s5_auth_success_total` | Counter | Total successful authentications |
| `s5_auth_failure_total` | Counter | Total failed authentication attempts |
| `s5_bans_total` | Counter | Total IP bans issued |
| `s5_acl_denied_total` | Counter | Total ACL-denied connections |
| `s5_audit_events_dropped_total` | Counter | Audit events lost due to channel overflow |
...
**Overview row:**
- Active connections (gauge, `s5_connections_active`)
- Connections per second (rate, `rate(s5_connections_total[5m])`)
- Auth failure rate (rate, `rate(s5_auth_failure_total[5m])`)
- Active bans count (gauge from API)

**Traffic row:**
- Bandwidth sent/received (rate, `rate(s5_bytes_sent_total[5m])`)
- Bandwidth by user (stacked area, `rate(s5_bytes_sent_total[5m])` grouped by user label)
- Connection duration histogram

**Security row:**
- Auth failures over time (time series, `rate(s5_auth_failure_total[1m])`)
- Ban events (time series, `rate(s5_bans_total[5m])`)
- ACL denials (time series, `rate(s5_acl_denied_total[5m])`)
- Dropped audit events (counter, `s5_audit_events_dropped_total`)
...
s5 has a built-in alerting engine. Configure alert rules to fire webhooks:
...
```bash
# Backup to a file
s5 backup --token YOUR_API_TOKEN --output backup.json

# Backup with custom API address
s5 backup --token YOUR_API_TOKEN --api-addr http://10.0.0.1:9091 --output backup.json

# Backup to stdout (for piping)
s5 backup --token YOUR_API_TOKEN
```
...
```bash
# Restore from a file
s5 restore --token YOUR_API_TOKEN --input backup.json

# Restore with custom API address
s5 restore --token YOUR_API_TOKEN --api-addr http://10.0.0.1:9091 --input backup.json
```
...
0 3 * * * /usr/local/bin/s5 backup --token YOUR_TOKEN --output /var/backups/s5/backup-$(date +\%Y\%m\%d).json 2>/dev/null

# Clean up backups older than 30 days
0 4 * * * find /var/backups/s5 -name "backup-*.json" -mtime +30 -delete
```

Ensure the backup directory exists and has appropriate permissions:

```bash
sudo mkdir -p /var/backups/s5
sudo chown s5:s5 /var/backups/s5
sudo chmod 750 /var/backups/s5
```
...
s5 is designed as a single-instance server. It keeps all state (connections, bans, quotas, sessions) in-memory. There is no built-in clustering or state replication.
...
This restarts s5 within 5 seconds if it crashes. The `on-failure` policy restarts only on non-zero exit codes (not on clean shutdown via SIGTERM).

### Graceful Shutdown

s5 supports graceful shutdown. When receiving SIGTERM:
...
If placing s5 behind a load balancer:
...
- Run multiple independent s5 instances behind a TCP load balancer
