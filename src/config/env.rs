...
//! 1. `S5_CONFIG` env var to specify config file path
...
/// Requires at minimum: S5_SSH_LISTEN + auth credentials (single or multi-user).
pub fn can_build_from_env() -> bool {
    let has_ssh = std::env::var("S5_SSH_LISTEN").is_ok();
    if !has_ssh {
        return false;
    }
    // Multi-user indexed mode
    if std::env::var("S5_USER_0_USERNAME").is_ok() {
        return true;
    }
    // Single-user mode
    std::env::var("S5_PASSWORD_HASH").is_ok()
        || std::env::var("S5_PASSWORD_HASH_FILE").is_ok()
        || std::env::var("S5_AUTHORIZED_KEYS").is_ok()
}

/// Build a complete AppConfig from environment variables.
/// Supports both single-user (flat vars) and multi-user (indexed S5_USER_<N>_*) modes.
pub fn build_config_from_env() -> anyhow::Result<AppConfig> {
    let ssh_listen = require_env("S5_SSH_LISTEN")?;

    // Determine user list: multi-user indexed mode vs single-user flat mode
    let users = if std::env::var("S5_USER_0_USERNAME").is_ok() {
    ...
    let config = AppConfig {
        server: ServerConfig {
            ssh_listen,
            socks5_listen: opt_env("S5_SOCKS5_LISTEN"),
            ...
            allowed_ciphers: parse_csv_env("S5_ALLOWED_CIPHERS"),
            allowed_kex: parse_csv_env("S5_ALLOWED_KEX"),
            shutdown_timeout: parse_env("S5_SHUTDOWN_TIMEOUT", 30),
            socks5_tls_cert: opt_env("S5_SOCKS5_TLS_CERT").map(PathBuf::from),
            socks5_tls_key: opt_env("S5_SOCKS5_TLS_KEY").map(PathBuf::from),
            dns_cache_ttl: parse_env("S5_DNS_CACHE_TTL", -1),
            dns_cache_max_entries: parse_env("S5_DNS_CACHE_MAX_ENTRIES", 1000),
            connect_retry: parse_env("S5_CONNECT_RETRY", 0),
            connect_retry_delay_ms: parse_env("S5_CONNECT_RETRY_DELAY_MS", 1000),
            bookmarks_path: opt_env("S5_BOOKMARKS_PATH").map(PathBuf::from),
            ssh_keepalive_interval_secs: parse_env("S5_SSH_KEEPALIVE_INTERVAL", 15),
            ssh_keepalive_max: parse_env("S5_SSH_KEEPALIVE_MAX", 3),
            ssh_auth_timeout: parse_env("S5_SSH_AUTH_TIMEOUT", 120),
        },
        shell: ShellConfig {
            hostname: opt_env("S5_SHELL_HOSTNAME").unwrap_or_else(|| "s5-proxy".to_string()),
            prompt: opt_env("S5_SHELL_PROMPT").unwrap_or_else(|| "$ ".to_string()),
            colors: parse_bool_env("S5_SHELL_COLORS", true),
            autocomplete: parse_bool_env("S5_SHELL_AUTOCOMPLETE", true),
        },
        limits: LimitsConfig {
            max_connections: parse_env("S5_MAX_CONNECTIONS", 1000),
            max_connections_per_user: parse_env("S5_MAX_CONNECTIONS_PER_USER", 0),
            connection_timeout: parse_env("S5_CONNECTION_TIMEOUT", 300),
            idle_timeout: parse_env("S5_IDLE_TIMEOUT", 0),
            max_auth_attempts: parse_env("S5_MAX_AUTH_ATTEMPTS", 3),
            socks5_handshake_timeout: parse_env("S5_SOCKS5_HANDSHAKE_TIMEOUT", 30),
        ...
        security: SecurityConfig {
            allowed_source_ips: opt_env("S5_ALLOWED_SOURCE_IPS")
                .map(|s| {
                    s.split(',')
                ...
                .transpose()
                .map_err(|e| anyhow::anyhow!("invalid S5_ALLOWED_SOURCE_IPS: {e}"))?
                .unwrap_or_default(),
            ban_enabled: parse_bool_env("S5_BAN_ENABLED", true),
            ban_threshold: parse_env("S5_BAN_THRESHOLD", 5),
            ban_window: parse_env("S5_BAN_WINDOW", 300),
            ban_duration: parse_env("S5_BAN_DURATION", 900),
            ...
            argon2_memory_cost: parse_env("S5_ARGON2_MEMORY_COST", 19456),
            argon2_time_cost: parse_env("S5_ARGON2_TIME_COST", 2),
            argon2_parallelism: parse_env("S5_ARGON2_PARALLELISM", 1),
            rate_limit_cleanup_interval: parse_env("S5_RATE_LIMIT_CLEANUP_INTERVAL", 60),
            rate_limit_max_ips: parse_env("S5_RATE_LIMIT_MAX_IPS", 100_000),
            rate_limit_max_users: parse_env("S5_RATE_LIMIT_MAX_USERS", 10_000),
        },
        logging: LoggingConfig {
            level: opt_env("S5_LOG_LEVEL")
                .map(|s| parse_log_level(&s))
                .transpose()?
                .unwrap_or(LogLevel::Info),
            format: opt_env("S5_LOG_FORMAT")
                .map(|s| parse_log_format(&s))
                .transpose()?
                .unwrap_or(LogFormat::Pretty),
            audit_log_path: opt_env("S5_AUDIT_LOG_PATH").map(PathBuf::from),
            audit_max_size_mb: parse_env("S5_AUDIT_MAX_SIZE_MB", 100),
            audit_max_files: parse_env("S5_AUDIT_MAX_FILES", 5),
            connection_flow_logs: parse_bool_env("S5_CONNECTION_FLOW_LOGS", false),
        },
        ...
        geoip: GeoIpConfig {
            enabled: parse_bool_env("S5_GEOIP_ENABLED", false),
            database_path: opt_env("S5_GEOIP_DATABASE_PATH").map(PathBuf::from),
            allowed_countries: parse_csv_env("S5_GEOIP_ALLOWED_COUNTRIES"),
            denied_countries: parse_csv_env("S5_GEOIP_DENIED_COUNTRIES"),
            fail_closed: parse_bool_env("S5_GEOIP_FAIL_CLOSED", false),
        },
        upstream_proxy: opt_env("S5_UPSTREAM_PROXY_URL").map(|url| UpstreamProxyConfig { url }),
        webhooks: Vec::new(),
        acl: GlobalAclConfig {
            default_policy: opt_env("S5_GLOBAL_ACL_DEFAULT_POLICY")
                .map(|s| parse_acl_policy(&s))
                .transpose()?
                .unwrap_or(AclPolicyConfig::Allow),
            allow: parse_csv_env("S5_GLOBAL_ACL_ALLOW"),
            deny: parse_csv_env("S5_GLOBAL_ACL_DENY"),
        },
        users,
        groups: Vec::new(),
...
/// For single-user mode, prefix is "S5_", for indexed mode it's "S5_USER_N_".
...
fn build_single_user_from_env() -> anyhow::Result<UserConfig> {
    let password_hash = resolve_env_or_file("S5_PASSWORD_HASH")?;
    ...
    let username = opt_env("S5_USERNAME").unwrap_or_else(|| "user".to_string());

    // Single-user mode uses "S5_" prefix (with some legacy names)
    let mut user = build_user_config("S5_", username, password_hash, authorized_keys)?;
    // Legacy: single-user upstream proxy uses a different env name
    if user.upstream_proxy.is_none() {
        user.upstream_proxy = opt_env("S5_USER_UPSTREAM_PROXY");
    }
    // Legacy: single-user max_connections uses different key
    if user.max_connections.is_none() {
        user.max_connections = opt_env("S5_MAX_CONNECTIONS_USER").map(|v| v.parse().unwrap_or(0));
    }
    Ok(user)
}

/// Collect indexed users from S5_USER_0_*, S5_USER_1_*, etc.
/// Stops at the first missing index.
fn collect_indexed_users() -> anyhow::Result<Vec<UserConfig>> {
    let mut users = Vec::new();
    for idx in 0u32.. {
        let prefix = format!("S5_USER_{idx}_");
    ...
    if users.is_empty() {
        anyhow::bail!("S5_USER_0_USERNAME is set but no valid indexed users found");
    }
...
/// e.g. prefix="S5_RATE_LIMIT" -> S5_RATE_LIMIT_PER_SECOND, S5_RATE_LIMIT_PER_MINUTE, S5_RATE_LIMIT_PER_HOUR
...
pub fn apply_env_overrides(config: &mut AppConfig) {
    // Server overrides
    if let Some(v) = opt_env("S5_SSH_LISTEN") {
        config.server.ssh_listen = v;
    }
    if let Some(v) = opt_env("S5_SOCKS5_LISTEN") {
        config.server.socks5_listen = Some(v);
    }
    if let Some(v) = opt_env("S5_HOST_KEY_PATH") {
        config.server.host_key_path = PathBuf::from(v);
    }
    if let Some(v) = opt_env("S5_BANNER") {
        config.server.banner = v;
    }
    if std::env::var("S5_PROXY_PROTOCOL").is_ok() {
        config.server.proxy_protocol = parse_bool_env("S5_PROXY_PROTOCOL", false);
    }
    ...
    // Logging overrides
    if let Some(v) = opt_env("S5_LOG_LEVEL") {
        if let Ok(level) = parse_log_level(&v) {
            config.logging.level = level;
        }
    }
    if let Some(v) = opt_env("S5_LOG_FORMAT") {
    ...
    if std::env::var("S5_CONNECTION_TIMEOUT").is_ok() {
        config.limits.connection_timeout =
            parse_env("S5_CONNECTION_TIMEOUT", config.limits.connection_timeout);
    }
    if std::env::var("S5_IDLE_TIMEOUT").is_ok() {
        config.limits.idle_timeout = parse_env("S5_IDLE_TIMEOUT", config.limits.idle_timeout);
    }
    if std::env::var("S5_MAX_BANDWIDTH_MBPS").is_ok() {
    ...
    // Security overrides
    if std::env::var("S5_BAN_ENABLED").is_ok() {
        config.security.ban_enabled = parse_bool_env("S5_BAN_ENABLED", config.security.ban_enabled);
    }
    if std::env::var("S5_BAN_THRESHOLD").is_ok() {
        config.security.ban_threshold =
            parse_env("S5_BAN_THRESHOLD", config.security.ban_threshold);
    }
    ...
    if std::env::var("S5_ARGON2_MEMORY_COST").is_ok() {
        config.security.argon2_memory_cost =
            parse_env("S5_ARGON2_MEMORY_COST", config.security.argon2_memory_cost);
    }
    if std::env::var("S5_ARGON2_TIME_COST").is_ok() {
        config.security.argon2_time_cost =
            parse_env("S5_ARGON2_TIME_COST", config.security.argon2_time_cost);
    }
    if std::env::var("S5_ARGON2_PARALLELISM").is_ok() {
        config.security.argon2_parallelism =
            parse_env("S5_ARGON2_PARALLELISM", config.security.argon2_parallelism);
    }
    ...
    if let Ok(Some(v)) = resolve_env_or_file("S5_API_TOKEN") {
        config.api.token = v;
    }
    if std::env::var("S5_API_ENABLED").is_ok() {
        config.api.enabled = parse_bool_env("S5_API_ENABLED", config.api.enabled);
    }
    if let Some(v) = opt_env("S5_API_LISTEN") {
        config.api.listen = v;
    }

    // Metrics overrides
    if std::env::var("S5_METRICS_ENABLED").is_ok() {
        config.metrics.enabled = parse_bool_env("S5_METRICS_ENABLED", config.metrics.enabled);
    }
    if let Some(v) = opt_env("S5_METRICS_LISTEN") {
        config.metrics.listen = v;
    }

    // Global ACL overrides
    if let Some(v) = opt_env("S5_GLOBAL_ACL_DEFAULT_POLICY") {
        if let Ok(policy) = parse_acl_policy(&v) {
            config.acl.default_policy = policy;
        }
    }
    if std::env::var("S5_GLOBAL_ACL_ALLOW").is_ok() {
        config.acl.allow = parse_csv_env("S5_GLOBAL_ACL_ALLOW");
    }
    if std::env::var("S5_GLOBAL_ACL_DENY").is_ok() {
        config.acl.deny = parse_csv_env("S5_GLOBAL_ACL_DENY");
    }
}
...
mod tests {
    ...
    fn with_env_vars<F: FnOnce()>(vars: &[(&str, &str)], f: F) {
        ...
        for key in [
            "S5_SSH_LISTEN",
            "S5_PASSWORD_HASH",
            "S5_PASSWORD_HASH_FILE",
            "S5_AUTHORIZED_KEYS",
            "S5_USERNAME",
            "S5_SOCKS5_LISTEN",
            "S5_LOG_LEVEL",
            "S5_LOG_FORMAT",
            "S5_ACL_DEFAULT_POLICY",
            "S5_ACL_ALLOW",
            "S5_ACL_DENY",
            "S5_API_TOKEN",
            "S5_API_TOKEN_FILE",
            ...
            "S5_METRICS_ENABLED",
            "S5_BAN_ENABLED",
            "S5_GLOBAL_ACL_DEFAULT_POLICY",
            "S5_GLOBAL_ACL_ALLOW",
            "S5_GLOBAL_ACL_DENY",
            "S5_TOTP_SECRET",
            "S5_TOTP_SECRET_FILE",
            "S5_USER_0_USERNAME",
            "S5_USER_0_PASSWORD_HASH",
            "S5_USER_0_PASSWORD_HASH_FILE",
            "S5_USER_0_AUTHORIZED_KEYS",
            "S5_USER_0_ALLOW_SHELL",
            "S5_USER_0_ACL_DEFAULT_POLICY",
            "S5_USER_0_ACL_ALLOW",
            "S5_USER_1_USERNAME",
            "S5_USER_1_PASSWORD_HASH",
            "S5_USER_1_ALLOW_FORWARDING",
            "S5_USER_1_ALLOW_SHELL",
            "S5_ARGON2_MEMORY_COST",
            "S5_ARGON2_TIME_COST",
            "S5_ARGON2_PARALLELISM",
        ] {
            std::env::remove_var(key);
        }
    }
    ...
    fn test_can_build_from_env_true_with_password() {
        with_env_vars(
            &[
                ...
                ("S5_PASSWORD_HASH", "argon2id-fake"),
            ],
            || {
                assert!(can_build_from_env());
            },
    ...
    fn test_can_build_from_env_true_with_keys() {
        with_env_vars(
            &[
                ("S5_SSH_LISTEN", "0.0.0.0:2222"),
                ("S5_AUTHORIZED_KEYS", "ssh-ed25519 AAAA..."),
            ],
            || {
                assert!(can_build_from_env());
            },
    ...
    fn test_build_config_full() {
        with_env_vars(
            &[
                ...
                ("S5_LOG_LEVEL", "debug"),
                ("S5_LOG_FORMAT", "json"),
                ("S5_ACL_DEFAULT_POLICY", "deny"),
                ("S5_ACL_ALLOW", "*.example.com:443,api.github.com:443"),
                ("S5_ACL_DENY", "evil.com:*"),
    ...
    fn test_build_config_missing_auth_fails() {
        with_env_vars(&[("S5_SSH_LISTEN", "0.0.0.0:2222")], || {
    ...
    fn test_apply_env_overrides() {
        with_env_vars(
            &[
                ("S5_LOG_LEVEL", "debug"),
                ("S5_API_TOKEN", "secret-from-env"),
                ("S5_GLOBAL_ACL_DENY", "10.0.0.0/8:*"),
            ],
            || {
                let toml_str = r##"
[server]
...
"##;
    ...
    fn test_parse_csv_env_empty() {
        with_env_vars(&[], || {
            assert!(parse_csv_env("S5_NONEXISTENT").is_empty());
        });
    }

    #[test]
    fn test_parse_bool_env_variants() {
        with_env_vars(&[("S5_TEST_BOOL", "true")], || {
            assert!(parse_bool_env("S5_TEST_BOOL", false));
        });
        with_env_vars(&[("S5_TEST_BOOL", "1")], || {
            assert!(parse_bool_env("S5_TEST_BOOL", false));
        });
        with_env_vars(&[("S5_TEST_BOOL", "yes")], || {
            assert!(parse_bool_env("S5_TEST_BOOL", false));
        });
        with_env_vars(&[("S5_TEST_BOOL", "false")], || {
            assert!(!parse_bool_env("S5_TEST_BOOL", true));
        });
        with_env_vars(&[("S5_TEST_BOOL", "0")], || {
            assert!(!parse_bool_env("S5_TEST_BOOL", true));
        });
    }
    ...
    fn test_can_build_from_env_multiuser() {
        with_env_vars(
            &[
                ...
                ("S5_USER_0_PASSWORD_HASH", "argon2id-fake"),
            ],
            || {
                assert!(can_build_from_env());
            },
    ...
    fn test_build_config_multiuser_two_users() {
        with_env_vars(
            &[
                ("S5_SSH_LISTEN", "0.0.0.0:2222"),
                ("S5_USER_0_USERNAME", "alice"),
                ("S5_USER_0_PASSWORD_HASH", "argon2id-fake-alice"),
                ("S5_USER_0_ALLOW_SHELL", "true"),
                ("S5_USER_0_ACL_DEFAULT_POLICY", "deny"),
                ("S5_USER_0_ACL_ALLOW", "*.example.com:443"),
                ("S5_USER_1_USERNAME", "bob"),
                ("S5_USER_1_PASSWORD_HASH", "argon2id-fake-bob"),
                ("S5_USER_1_ALLOW_FORWARDING", "false"),
    ...
    fn test_build_config_multiuser_stops_at_gap() {
        with_env_vars(
            &[
                ...
                ("S5_USER_0_USERNAME", "alice"),
                ("S5_USER_0_PASSWORD_HASH", "argon2id-fake"),
                // S5_USER_1_USERNAME is missing → only 1 user
            ],
            || {
                let config = build_config_from_env().unwrap();
                assert_eq!(config.users.len(), 1);
    ...
    fn test_multiuser_ignores_flat_vars() {
        // When S5_USER_0_USERNAME exists, flat vars (S5_USERNAME etc.) are ignored
        with_env_vars(
            &[
                ("S5_SSH_LISTEN", "0.0.0.0:2222"),
                ("S5_USERNAME", "flat-user"),
                ("S5_PASSWORD_HASH", "flat-hash"),
                ("S5_USER_0_USERNAME", "indexed-alice"),
                ("S5_USER_0_PASSWORD_HASH", "indexed-hash"),
            ],
            || {
                let config = build_config_from_env().unwrap();
                assert_eq!(config.users.len(), 1);
    ...
    fn test_resolve_env_or_file_direct() {
        with_env_vars(&[("S5_API_TOKEN", "direct-value")], || {
    ...
    fn test_resolve_env_or_file_missing_file_fails() {
        with_env_vars(
            &[("S5_API_TOKEN_FILE", "/nonexistent/path/secret.txt")],
            || {
                let result = resolve_env_or_file("S5_API_TOKEN");
                assert!(result.is_err());
            },
        );
    }
    ...
    fn test_build_config_password_hash_file() {
        ...
        with_env_vars(
            &[
                ("S5_SSH_LISTEN", "0.0.0.0:2222"),
                ("S5_PASSWORD_HASH_FILE", hash_path.to_str().unwrap()),
            ],
            || {
                let config = build_config_from_env().unwrap();
                assert_eq!(
