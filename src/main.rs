...
fn main() -> Result<()> {
    ...
    match &cli.command {
        Some(Command::HashPassword { password }) => {
            return s5::auth::password::hash_password_cli(password.as_deref());
        }
        ...
        Some(Command::QuickStart {
            ...
            let password = match password {
                Some(p) => p.clone(),
                None => {
                    let generated = s5::auth::password::generate_password(20);
                    eprintln!("Generated password: {}", generated);
                    generated
                }
            };

            let password_hash = s5::auth::password::hash_password(&password)?;
            ...
            info!(
                ...
                "Starting s5 proxy server (quick-start)"
            );

            let rt = tokio::runtime::Runtime::new()?;
            rt.block_on(async {
                if let Err(e) = s5::server::run(app_config).await {
        ...
        Some(Command::SshConfig {
            ...
            let alias = name.as_deref().unwrap_or("s5-proxy");
            println!("# s5 SSH config snippet");
        ...
        Some(Command::Completions { shell }) => {
            ...
            clap_complete::generate(*shell, &mut cmd, "s5", &mut std::io::stdout());
            return Ok(());
        }
        ...
        Some(Command::Demo {
            ...
            let password_hash = s5::auth::password::hash_password(password)?;
            let demo_config =
                s5::demo::build_demo_config(*ssh_port, *socks5_port, *api_port, &password_hash);
            ...
            info!(
                ...
                api_port = api_port,
                "Starting s5 demo server"
            );

            let rt = tokio::runtime::Runtime::new()?;
            rt.block_on(async {
                if let Err(e) =
                    s5::server::run_with_post_init(demo_config, None, |ctx| async move {
                        // Inject demo data; keep sessions alive for server lifetime
                        let _sessions = s5::demo::inject_demo_data(&ctx).await;
    ...
    info!(
        version = env!("CARGO_PKG_VERSION"),
        ssh_listen = %app_config.server.ssh_listen,
        "Starting s5 proxy server"
    );

    // Run the async server
    let rt = tokio::runtime::Runtime::new()?;
    rt.block_on(async {
        if let Err(e) = s5::server::run_with_config_path(app_config, config_path).await {
            error!(error = %e, "Server error");
            std::process::exit(1);
        }
    });
...
fn build_quick_config(
    ...
    AppConfig {
        server: ServerConfig {
            ...
            server_id: "SSH-2.0-s5".to_string(),
            banner: "Welcome to s5".to_string(),
            motd_path: None,
...
server_id = "SSH-2.0-s5"
banner = "Welcome to s5"
proxy_protocol = false
