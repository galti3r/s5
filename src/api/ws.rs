        ...
        "kick" => {
            if let Some(username) = &cmd.username {
                ...
                if auth.user_store().get(username).is_some() {
                    ...
                    WsResponse { success: true, action: "kick".to_string(), error: None }
                } else {
                    WsResponse { success: false, action: "kick".to_string(), error: Some("user not found".to_string()) }
                }
            } else {
                WsResponse { success: false, action: "kick".to_string(), error: Some("username required".to_string()) }
            }
        }
        "maintenance" => {
            let enabled = cmd.enabled.unwrap_or(true);
            state.maintenance.store(enabled, std::sync::atomic::Ordering::Relaxed);
            WsResponse { success: true, action: "maintenance".to_string(), error: None }
        }
        "unban" => {
            if let Some(ip_str) = &cmd.ip {
                if let Ok(ip) = ip_str.parse() {
                    ...
                    WsResponse { success: true, action: "unban".to_string(), error: None }
                } else {
                    WsResponse { success: false, action: "unban".to_string(), error: Some("invalid IP".to_string()) }
                }
            } else {
                WsResponse { success: false, action: "unban".to_string(), error: Some("ip required".to_string()) }
            }
        }
        "broadcast" => {
            if let Some(message) = &cmd.message {
                ...
                WsResponse { success: true, action: "broadcast".to_string(), error: None }
            } else {
                WsResponse { success: false, action: "broadcast".to_string(), error: Some("message required".to_string()) }
            }
        }
        _ => {
