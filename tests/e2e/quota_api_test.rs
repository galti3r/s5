...
use s5::quota::{QuotaResult, QuotaTracker};
use s5::security::SecurityManager;
use std::sync::Arc;
use tokio::sync::RwLock;

fn make_config(api_port: u16, hash: &str) -> s5::config::types::AppConfig {
    let toml_str = format!(
        r##"
[server]
ssh_listen = "127.0.0.1:0"
host_key_path = "/tmp/s5-e2e-quota-api-key"
...
async fn test_quota_list_after_activity() {
    ...
    match qt.record_bytes("alice", 5000, 0, 0, None) {
        s5::quota::QuotaResult::Ok(_) => {}
        s5::quota::QuotaResult::Exceeded(r) => panic!("unexpected: {r}"),
    }
...
[server]
ssh_listen = "127.0.0.1:0"
host_key_path = "/tmp/s5-e2e-quota-metrics-key"
...
"##
    ...
    assert!(
        body.contains("s5_quota_bandwidth_used_bytes"),
        "should have quota bandwidth metric"
    );
    assert!(
        body.contains("s5_quota_connections_used"),
        "should have quota connections metric"
    );
    assert!(
        body.contains("s5_quota_exceeded_total"),
