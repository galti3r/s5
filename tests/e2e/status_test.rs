...
host_key_path = "/tmp/s5-e2e-status-key"
...
"##
    ...
    let metrics = Arc::new(s5::metrics::MetricsRegistry::new());
...
[server]
ssh_listen = "127.0.0.1:0"
host_key_path = "/tmp/s5-e2e-metrics-key"
...
"##
    ...
    assert!(
        body.contains("s5_auth_failures_total"),
        "metrics should contain auth failures"
    );
    assert!(
        body.contains("s5_auth_successes_total"),
...
[server]
ssh_listen = "127.0.0.1:0"
host_key_path = "/tmp/s5-e2e-maint-key"
