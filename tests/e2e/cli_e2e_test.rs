...
fn test_show_config_toml_format() {
    ...
    let output = Command::cargo_bin("s5")
        .unwrap()
        .args(["-c", path, "show-config"])
        .output()
        .expect("failed to run s5 show-config");
...
fn test_show_config_json_format() {
    ...
    let output = Command::cargo_bin("s5")
        ...
        .output()
        .expect("failed to run s5 show-config --format json");
...
fn test_init_creates_config() {
    ...
    let out_path = dir.path().join("s5-test-init.toml");
...
fn test_init_bastion_preset() {
    let dir = tempfile::tempdir().expect("failed to create temp dir");
    let out_path = dir.path().join("s5-test-bastion.toml");
    let out_str = out_path.to_str().unwrap();
    ...
    let output = Command::cargo_bin("s5")
        ...
        .output()
        .expect("failed to run s5 init --preset bastion");
...
fn test_init_proxy_preset() {
    let dir = tempfile::tempdir().expect("failed to create temp dir");
    let out_path = dir.path().join("s5-test-proxy.toml");
    let out_str = out_path.to_str().unwrap();
    ...
    let output = Command::cargo_bin("s5")
        ...
        .output()
        .expect("failed to run s5 init --preset proxy");
...
fn test_init_dev_preset() {
    let dir = tempfile::tempdir().expect("failed to create temp dir");
    let out_path = dir.path().join("s5-test-dev.toml");
    let out_str = out_path.to_str().unwrap();
    ...
    let output = Command::cargo_bin("s5")
        ...
        .output()
        .expect("failed to run s5 init --preset dev");
...
fn test_completions_bash() {
    let output = Command::cargo_bin("s5")
        ...
        .output()
        .expect("failed to run s5 completions bash");
...
fn test_manpage_output() {
    let output = Command::cargo_bin("s5")
        ...
        .args(["manpage"])
        .output()
        .expect("failed to run s5 manpage");
...
fn test_check_config_valid() {
    ...
    let output = Command::cargo_bin("s5")
        ...
        .output()
        .expect("failed to run s5 check-config");
...
fn test_check_config_invalid() {
    ...
    let output = Command::cargo_bin("s5")
        ...
        .args(["-c", path, "check-config"])
        .output()
        .expect("failed to run s5 check-config with invalid config");
