...
async fn ensure_chrome() -> u16 {
    CHROME
        .get_or_init(|| async {
            ...
            let container_name = format!("s5-chrome-{}", std::process::id());
...
async fn test_dashboard_page_loads() {
    ...
    assert_eq!(title.as_deref(), Some("s5 Dashboard"), "page title");

    let h1_text = eval_str(&page, "document.querySelector('header h1').textContent").await;
    assert!(
        h1_text.contains("s5 Dashboard"),
        "h1 should contain 's5 Dashboard', got: {}",
        h1_text
    );
