...
fn critical_events_are_classified_correctly() {
    ...
    // Critical events
    let auth_failure = AuditEvent::auth_failure("user", &addr, "password");
    assert!(auth_failure.is_critical(), "AuthFailure should be critical");

    let acl_deny = AuditEvent::acl_deny("user", "evil.com", 80, None, "1.2.3.4", None, "blocked");
...
async fn critical_events_delivered_under_normal_load() {
    ...
    logger.log_auth_failure("attacker", &addr, "password").await;
    logger.log_connection_new(&addr, "ssh");
    logger.log_acl_deny("user", "evil.com", 443, None, "10.0.0.1", None, "blocked");
...
async fn noop_logger_does_not_panic_on_critical_events() {
    ...
    logger.log_auth_failure("user", &addr, "password").await;
    logger.log_acl_deny("user", "evil.com", 80, None, "10.0.0.1", None, "blocked");
    logger.log_config_reload(1, false, Some("test error".to_string()));
    logger.log_connection_new(&addr, "socks5");
    logger.log_connection_closed(&addr, "socks5");
