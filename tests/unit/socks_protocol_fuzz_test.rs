...
proptest! {
    ...
    fn greeting_random_bytes_never_panics(data in proptest::collection::vec(any::<u8>(), 0..256)) {
        ...
        rt.block_on(async {
            let mut cursor = &data[..];
            let _ = s5::socks::protocol::read_greeting(&mut cursor).await;
        });
    }
    ...
    fn valid_greeting_parses_correctly(
        ...
        rt.block_on(async {
            ...
            let mut cursor = &data[..];
            let result = s5::socks::protocol::read_greeting(&mut cursor).await;
    ...
    fn connect_request_random_bytes_never_panics(data in proptest::collection::vec(any::<u8>(), 4..512)) {
        ...
        rt.block_on(async {
            ...
            let _ = s5::socks::protocol::read_connect_request(&mut client).await;
        });
    }
    ...
    fn valid_domain_connect_parses(
        ...
        rt.block_on(async {
            ...
            let result = s5::socks::protocol::read_connect_request(&mut client).await;
            match result {
                Ok(target) => {
