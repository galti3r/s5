...
use s5::ssh::handler::{classify_relay_error, SshHandler, MAX_CHANNELS_PER_CONNECTION};
...
async fn record_auth_failure_below_max_allows_retry() {
    ...
    match result {
        russh::server::Auth::Reject {
            ...
            assert!(
                methods.contains(russh::MethodSet::PASSWORD),
                "should include PASSWORD method"
            );
            assert!(
                methods.contains(russh::MethodSet::PUBLICKEY),
