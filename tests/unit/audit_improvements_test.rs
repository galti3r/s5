...
async fn webhook_client_does_not_follow_redirects() {
    ...
    let config = s5::config::types::WebhookConfig {
    ...
    let dispatcher = s5::webhooks::WebhookDispatcher::new(vec![config]);
...
fn user_store_get_returns_arc() {
    ...
    let store = s5::auth::user::UserStore::from_config(
        &configs,
        &[],
        &GlobalAclConfig::default(),
        &s5::config::types::LimitsConfig::default(),
        &server,
        &s5::config::types::ShellConfig::default(),
    )
    .unwrap();
    ...
    let arc_clone: Arc<s5::auth::user::User> = Arc::clone(arc_ref);
...
fn metrics_prune_known_users_removes_stale() {
    let metrics = s5::metrics::MetricsRegistry::new();
...
async fn relay_handles_zero_traffic_gracefully() {
    use s5::proxy::forwarder::{relay, RelayConfig};
