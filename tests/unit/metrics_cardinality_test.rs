...
fn connection_duration_appears_in_prometheus() {
    ...
    assert!(
        buffer.contains("s5_connection_duration_seconds"),
        "Prometheus output should contain s5_connection_duration_seconds, got:\n{}",
        buffer
    );
    assert!(
        buffer.contains("alice"),
...
fn connection_duration_histogram_buckets() {
    ...
    assert!(
        buffer.contains("s5_connection_duration_seconds_bucket"),
        "Should contain histogram bucket lines"
    );
    assert!(
        buffer.contains("s5_connection_duration_seconds_sum"),
        "Should contain histogram sum"
    );
    assert!(
        buffer.contains("s5_connection_duration_seconds_count"),
