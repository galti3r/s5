...
fn generate_and_load_round_trip() {
    ...
    assert!(
        matches!(&key1, russh_keys::key::KeyPair::Ed25519(..)),
        "generated key should be Ed25519"
    );
    assert!(
        matches!(&key2, russh_keys::key::KeyPair::Ed25519(..)),
...
fn load_existing_key() {
    ...
    assert!(matches!(&key1, russh_keys::key::KeyPair::Ed25519(..)));
    assert!(matches!(&key2, russh_keys::key::KeyPair::Ed25519(..)));
}
