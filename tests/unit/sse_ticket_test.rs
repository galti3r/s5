...
fn valid_ticket_accepted() {
    ...
    assert!(s5::api::verify_sse_ticket(&ticket, token));
}
...
fn tampered_signature_rejected() {
    ...
    assert!(!s5::api::verify_sse_ticket(&tampered, token));
}
...
fn wrong_api_token_rejected() {
    ...
    assert!(!s5::api::verify_sse_ticket(&ticket, "bbb"));
}

#[test]
fn malformed_ticket_no_colon() {
    assert!(!s5::api::verify_sse_ticket("notavalidticket", "token"));
}

#[test]
fn malformed_ticket_bad_timestamp() {
    assert!(!s5::api::verify_sse_ticket("abc:123:hexhex", "token"));
}

#[test]
fn empty_ticket_rejected() {
    assert!(!s5::api::verify_sse_ticket("", "token"));
}
...
fn empty_token_ticket() {
    ...
    assert!(s5::api::verify_sse_ticket(&ticket, ""));
}
