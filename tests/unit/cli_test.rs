...
fn default_config_path() {
    let cli = Cli::try_parse_from(["s5"]).unwrap();
...
fn custom_config_short_flag() {
    let cli = Cli::try_parse_from(["s5", "-c", "/etc/s5/custom.toml"]).unwrap();
    assert_eq!(cli.config.to_str().unwrap(), "/etc/s5/custom.toml");
}
...
fn custom_config_long_flag() {
    let cli = Cli::try_parse_from(["s5", "--config", "myconfig.toml"]).unwrap();
...
fn log_level_override() {
    let cli = Cli::try_parse_from(["s5", "--log-level", "debug"]).unwrap();
...
fn hash_password_subcommand_with_password() {
    let cli = Cli::try_parse_from(["s5", "hash-password", "-p", "mysecret"]).unwrap();
    match cli.command {
...
fn hash_password_subcommand_no_password() {
    let cli = Cli::try_parse_from(["s5", "hash-password"]).unwrap();
    match cli.command {
        Some(Command::HashPassword { password }) => {
...
fn check_config_subcommand() {
    let cli = Cli::try_parse_from(["s5", "check-config"]).unwrap();
    match cli.command {
...
fn unknown_subcommand_rejected() {
    let result = Cli::try_parse_from(["s5", "unknown-command"]);
...
#[test]
fn version_flag() {
    let result = Cli::try_parse_from(["s5", "--version"]);
...
fn quick_start_defaults() {
    let cli = Cli::try_parse_from(["s5", "quick-start"]).unwrap();
    match cli.command {
...
#[test]
fn init_defaults() {
    let cli = Cli::try_parse_from(["s5", "init"]).unwrap();
    match cli.command {
        Some(Command::Init {
            output,
            username,
...
fn completions_subcommand() {
    let cli = Cli::try_parse_from(["s5", "completions", "bash"]).unwrap();
...
fn manpage_subcommand() {
    let cli = Cli::try_parse_from(["s5", "manpage"]).unwrap();
...
fn show_config_subcommand() {
    let cli = Cli::try_parse_from(["s5", "show-config"]).unwrap();
    match cli.command {
...
fn show_config_json_format() {
    let cli = Cli::try_parse_from(["s5", "show-config", "--format", "json"]).unwrap();
    match cli.command {
        Some(Command::ShowConfig { format }) => {
...
fn init_short_output_flag() {
    let cli = Cli::try_parse_from(["s5", "init", "-o", "short.toml"]).unwrap();
    match cli.command {
