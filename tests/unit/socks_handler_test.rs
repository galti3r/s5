...
host_key_path = "/tmp/s5-test-host-key"
...
async fn no_password_method_rejected() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        s5::socks::handler::handle_connection(stream, ctx).await
    });
...
"##
    ...
    let server1 = tokio::spawn(async move {
        let (stream, _) = listener.accept().await.unwrap();
        s5::socks::handler::handle_connection(stream, ctx1).await
    });
    ...
    let server2 = tokio::spawn(async move {
        ...
        s5::socks::handler::handle_connection(stream, ctx2).await
    });
...
async fn concurrent_connections_both_succeed() {
    ...
    let server_handle = tokio::spawn(async move {
        ...
        let h1 = tokio::spawn(s5::socks::handler::handle_connection(stream1, ctx1));
        let h2 = tokio::spawn(s5::socks::handler::handle_connection(stream2, ctx2));
