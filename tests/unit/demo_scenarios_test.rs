...
use s5::config::types::{AclPolicyConfig, ShellPermissions, UserRole};
use s5::demo::build_demo_config;
use s5::proxy::ProxyEngine;
use s5::quota::QuotaTracker;
use s5::shell::commands::{execute, CommandResult};
use s5::shell::context::ShellContext;
use s5::shell::filesystem::VirtualFs;
...
fn run_cmd(ctx: &mut ShellContext, fs: &mut VirtualFs, line: &str) -> CommandResult {
    ...
    execute(cmd, &args, fs, &username, "s5-demo", Some(ctx))
}
...
fn demo_show_status() {
    ...
    let mut fs = VirtualFs::new("alice", "s5-demo");
