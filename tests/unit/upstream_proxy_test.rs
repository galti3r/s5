use s5::config::types::ParsedUpstreamProxy;
...
mod resolve_priority {
    ...
    fn make_user(upstream: Option<&str>) -> s5::auth::user::User {
        use s5::config::acl::ParsedAcl;
        use s5::config::types::{AclPolicyConfig, RateLimitsConfig, ShellPermissions, UserRole};
    ...
    fn test_no_upstream_proxy() {
        ...
        let result = s5::proxy::ProxyEngine::resolve_upstream_proxy(&user, &config);
