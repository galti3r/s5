...
async fn test_metrics_prometheus_format() {
    ...
    assert!(body.contains("s5_auth_failures_total"));
    assert!(body.contains("s5_auth_successes_total"));
}
...
fn is_truthy_none_returns_false() {
    assert!(!s5::api::is_truthy(None));
}

#[test]
fn is_truthy_true_returns_true() {
    assert!(s5::api::is_truthy(Some("true")));
}

#[test]
fn is_truthy_one_returns_true() {
    assert!(s5::api::is_truthy(Some("1")));
}

#[test]
fn is_truthy_yes_returns_true() {
    assert!(s5::api::is_truthy(Some("yes")));
}

#[test]
fn is_truthy_false_returns_false() {
    assert!(!s5::api::is_truthy(Some("false")));
}

#[test]
fn is_truthy_zero_returns_false() {
    assert!(!s5::api::is_truthy(Some("0")));
}

#[test]
fn is_truthy_empty_returns_false() {
    assert!(!s5::api::is_truthy(Some("")));
}

#[test]
fn is_truthy_no_returns_false() {
    assert!(!s5::api::is_truthy(Some("no")));
}
...
fn make_ticket(api_token: &str, timestamp: u64) -> String {
    ...
    let signing_key = format!("s5-sse-ticket:{}", api_token);
...
fn verify_sse_ticket_replay_protection() {
    ...
    assert!(
        s5::api::verify_sse_ticket(&ticket, token),
...
fn build_test_app_state(api_token: &str) -> s5::api::AppState {
    let toml_str = format!(
        r##"
[server]
ssh_listen = "127.0.0.1:0"
host_key_path = "/tmp/s5-unit-api-test-key"
...
"##
    ...
    let config: s5::config::types::AppConfig = toml::from_str(&toml_str).unwrap();
    let config = Arc::new(config);
    let audit = Arc::new(s5::audit::AuditLogger::new(None, 0, 0, None));

    s5::api::AppState {
        auth_service: Arc::new(tokio::sync::RwLock::new(
            s5::auth::AuthService::new(&config).unwrap(),
        )),
        proxy_engine: Arc::new(s5::proxy::ProxyEngine::new(config.clone(), audit)),
        security: Arc::new(tokio::sync::RwLock::new(
            s5::security::SecurityManager::new(&config),
        )),
...
async fn start_full_api_server(api_token: &str) -> (u16, tokio_util::sync::CancellationToken) {
    ...
    tokio::spawn(async move {
        let _ = s5::api::start_api_server(&addr_clone, state, cancel_clone).await;
    });
